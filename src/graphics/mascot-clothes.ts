  ...
  constructor(body:SoftBody,group:THREE.Group) {
    ...
    const panel=(outline:number[][],hex:string)=>{
      ...
      for(let row=0;row<=2;row++)for(let col=0;col<=2;col++) {
        ...
        this.offsets.push(hex==='#a4aaa6'?.00065:hex==='#354f68'?.0005:.0003);
      }
      for(let row=0;row<2;row++)for(let col=0;col<2;col++) {
    ...
    panel([[-.0015,.0555],[-.009,.053],[-.006,.049],[-.002,.052]],'#354f68');
    panel([[.0015,.0555],[.002,.052],[.006,.049],[.009,.053]],'#354f68');
    for(const sign of [-1,1]) {
      const x=sign*.011;
      panel([[x-.004,.038],[x+.004,.038],[x+.004,.0445],[x-.004,.0445]],'#294861');
      panel([[x-.004,.045],[x+.004,.045],[x+.0035,.043],[x-.0035,.043]],'#354f68');
    }
