...
assert(!inputSource.includes('raycaster.intersectObject(this.mesh'), 'grab start must not scan the visible triangles linearly');
...
// Resolutions come from build-droplet.mjs: the visible lathe is geometry(96,64)
// and the optical proxy geometry(48,32), so 96*63+2 and 48*31+2 vertices. Pinning
// both counts catches a silent downgrade of the rendered mesh in either direction.
assert.equal(body.surface.positions.length/3,6050,'visible mesh remains at full resolution');
assert.equal(proxy.positions.length/3,1490,'optical proxy remains the coarse lathe');
assert(proxy.positions.length<body.surface.positions.length/3,'only the optical calculation uses the proxy');
