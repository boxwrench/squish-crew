...
export function buildOpticalModel(definitions,scale,bottom,full,sourceGeometry=null) {
  const reduced=sourceGeometry?null:definitions.replace('const NX = 104, NY = 96, NZ = 88;','const NX = 40, NY = 36, NZ = 34;');
  if(!sourceGeometry&&reduced===definitions)throw new Error('Reference polygonizer dimensions changed');
  const raw=sourceGeometry??new Function('THREE',reduced+'\nreturn buildJellyGeometry();')(THREE);
  ...
  for(let i=0;i<full.positions.length;i+=3) {
    ...
    let hit=bvh.hit(origin,dir,.008),rayOffset=.004;
    if(sourceGeometry) {
      // The concave neck can intersect its own normal ray. Radial projection
      // gives a consistent local mapping for this star-shaped surface.
      const radial=[full.positions[i],full.positions[i+1]-.025,full.positions[i+2]];
      const length=Math.hypot(...radial),direction=radial.map(v=>-v/length);
      const outside=[full.positions[i]-direction[0]*.01,full.positions[i+1]-direction[1]*.01,full.positions[i+2]-direction[2]*.01];
      outside[0]+=1e-8;outside[2]+=2e-8;
      hit=bvh.hit(outside,direction,.025);
      rayOffset=.01;
    }
    if(!hit)throw new Error(`No optical surface at vertex ${i/3}`);
    maxDistance=Math.max(maxDistance,Math.abs(hit.distance-rayOffset));
