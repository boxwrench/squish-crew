...
for(const {name,hz,grips,hold,useJS} of [
  ...
  const sound={
    unlock:async()=>{},
    hop(){},grab(){},stretch(){},stopStretch(){},release(){},
  };
  const input=new Input(camera,canvas,body,new Mesh(body.surface.geometry),rig,sound,()=>{});
  try {
    // Input's controls reframe the camera on construction, and eventRay refreshes
